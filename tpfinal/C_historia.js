...
mostrar() { console.log (this.estado);
   ...
   background(185, 210, 235);   // PANTALLA INICIO 
    if (this.estado === "0") { 
image(this.imgArray[0], width / 2 - 212.5, height / 5 - 50, 425, 450);
...
// gGANAR
  ...
  textSize(40);
  textAlign(CENTER, CENTER);
   text(this.textosArray[31], width / 2, height / 10);
   // textos
  fill(255, 255, 255); 
  textSize(20);
  text(this.textosArray[32], width / 2, height / 6);
this.boton10.mostrar();
}}

actualizar() {
   if (this.juego && this.juego.estado !== "jugando") {
    ...
    if (this.estado !== "jugando" && !this.juego) {
        ...
        else if (this.estado === "18" || this.estado === "19") {
 }}
  ...
  for (let boton of Boton.botones) {
    if (boton.Click()) {
      ...
      if (boton === Boton.boton1 && this.estado === "1") {
        ...
        this.estado = "17"; } if (this.estado === "17") {
  this.juego.iniciar();
  loop();
